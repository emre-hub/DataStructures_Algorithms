...
public class OgrListe {
    ...
    void ekle(){
        ...
        if(head == null){
            head = eleman;
            tail = eleman;
            System.out.println("Bilgisayar Bölümüne Kayıt Olacak Öğrencilerin Listesi Oluşturuldu ve İlk Öğrenci Kaydedildi.");
        }else{
            eleman.next = head;
            head = eleman;
    ...
    void sil(){
        if(head == null){
            ...
            if(numara == head.numara && head.next == null){
                ...
                if( numara == temp.numara){ //yukarıda taile gelince while'dan atıyor. kullanıcı tail'i silmek istiyorsa işlemi burada yapıyorum.
                    temp2.next = null;
                    tail = temp2;
                    System.out.println(numara + " numaralı öğrenci silindi. ");
                }
